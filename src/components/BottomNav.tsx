...
export default function BottomNav() {
  ...
  useEffect(() => {
    const check = () => {
      try { setIsAuthed(!!(typeof window !== 'undefined' && localStorage.getItem('token'))); } catch { setIsAuthed(false); }
    };
    check();
  ...
  function guardNav(e: React.MouseEvent, href: string) {
    if (!isAuthed && (href === "/wallet" || href === "/profile")) {
      e.preventDefault();
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white border-t border-gray-200 shadow-sm z-40">
      <div className="flex">
        <Link href="/wallet" onClick={(e) => guardNav(e, "/wallet")} className={`${baseItem} ${isActive("/wallet") ? activeCls : inactiveCls}`} aria-label="Ví tiền">
        ...
        <Link href="/profile" onClick={(e) => guardNav(e, "/profile")} className={`${baseItem} ${isActive("/profile") ? activeCls : inactiveCls}`} aria-label="Hồ sơ">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
